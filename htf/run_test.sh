...
cp ${PATHRT}/../../bin/ufs_model ufs_model
...
do
  atparse < ${PATHTR}/fv3_conf/${i} >> fv3_run
done

#
sed -i 's/PATHRT/PATHTR/g' fv3_run
...
if [[ "Q${INPUT_NEST03_NML:-}" != Q ]] ; then
    ...
    atparse < ${PATHTR}/parm/${INPUT_NEST03_NML} > input_nest03.nml
else
...
if [[ "Q${INPUT_NEST04_NML:-}" != Q ]] ; then
    ...
    atparse < ${PATHTR}/parm/${INPUT_NEST04_NML} > input_nest04.nml
else
...
if [[ "Q${INPUT_NEST05_NML:-}" != Q ]] ; then
    ...
    atparse < ${PATHTR}/parm/${INPUT_NEST05_NML} > input_nest05.nml
else
...
if [[ "Q${INPUT_NEST06_NML:-}" != Q ]] ; then
    ...
    atparse < ${PATHTR}/parm/${INPUT_NEST06_NML} > input_nest06.nml
else
...
if [[ "Q${DIAG_TABLE:-}" != Q ]] ; then
  cp ${PATHTR}/parm/diag_table/${DIAG_TABLE} diag_table
fi
# Field table
if [[ "Q${FIELD_TABLE:-}" != Q ]] ; then
  cp ${PATHTR}/parm/field_table/${FIELD_TABLE} field_table
fi
...
if [[ $CPLWAV == .true. ]]; then
  atparse < ${PATHTR}/parm/ww3_multi.inp.IN > ww3_multi.inp
fi

if [[ $DATM_CDEPS = 'true' ]] || [[ $S2S = 'true' ]]; then
  if [[ $HAFS = 'false' ]]; then
    atparse < ${PATHTR}/parm/ice_in_template > ice_in
    atparse < ${PATHTR}/parm/${MOM_INPUT:-MOM_input_template_$OCNRES} > INPUT/MOM_input
    atparse < ${PATHTR}/parm/diag_table/${DIAG_TABLE:-diag_table_template} > diag_table
    atparse < ${PATHTR}/parm/data_table_template > data_table
  fi
fi
...
if [[ "${DIAG_TABLE_ADDITIONAL:-}Q" != Q ]] ; then
  ...
  atparse < "${PATHTR}/parm/diag_table/${DIAG_TABLE_ADDITIONAL:-}" >> diag_table
fi

if [[ $DATM_CDEPS = 'true' ]]; then
  atparse < ${PATHTR}/parm/${DATM_IN_CONFIGURE:-datm_in} > datm_in
  atparse < ${PATHTR}/parm/${DATM_STREAM_CONFIGURE:-datm.streams.IN} > datm.streams
fi

if [[ $DOCN_CDEPS = 'true' ]]; then
  atparse < ${PATHTR}/parm/${DOCN_IN_CONFIGURE:-docn_in} > docn_in
  atparse < ${PATHTR}/parm/${DOCN_STREAM_CONFIGURE:-docn.streams.IN} > docn.streams
fi
...
if [[ $SCHEDULER = 'pbs' ]]; then
  ...
  atparse < $PATHTR/fv3_conf/fv3_qsub.IN > job_card
elif [[ $SCHEDULER = 'slurm' ]]; then
  NODES=$(( TASKS / TPN ))
  if (( NODES * TPN < TASKS )); then
    NODES=$(( NODES + 1 ))
  fi
  atparse < $PATHTR/fv3_conf/fv3_slurm.IN > job_card
  ...
  atparse < $PATHTR/fv3_conf/fv3_bsub.IN > job_card
fi
...
${MPI_EXE} -n ${TASKS} ./ufs_model 2> err 1> out
