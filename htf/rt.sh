...
readonly PATHTR=$( cd ${PATHRT}/../../src/ufs-weather-model/tests && pwd )
echo $PATHTR
...
# accounting code.
export MACHINE_ID="linux.$RT_COMPILER"
export ACCNR="${ACCNR:-compute}"
export SCHEDULER=""
...
#get data from aws
INPUTDATA_ROOT=${INPUTDATA_ROOT:-$PATHRT/input-data}
...
EOF
    ...
    [[ -e "${PATHTR}/tests/$TEST_NAME" ]] || die "run test file tests/$TEST_NAME does not exist"
    [[ $CREATE_BASELINE == true && $CB != *fv3* ]] && continue
